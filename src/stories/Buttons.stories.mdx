...
## CrossSellButton Buttons

CrossSellButton buttons are used as a call to action and indicate the most important action on a page.

<Story name="cross sell">
  <CrossSellButton onClick={action('button-click')} disabled={boolean("Disabled", false)}>Hello world</CrossSellButton>
</Story>